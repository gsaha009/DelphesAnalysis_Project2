...
template <class T>
bool ThisObjIsMuon(const T& obj, std::vector<Muon> muList, float dr) {
  TLorentzVector objP4 = obj.P4();
  for (const auto& mu : muList)
    if (objP4.DeltaR(mu.P4()) <= dr)
      return true;
  return false;
}

template <class T>
bool ThisObjIsElectron(const T& obj, std::vector<Electron> eleList, float dr) {
  TLorentzVector objP4 = obj.P4();
  for (const auto& ele : eleList)
    if (objP4.DeltaR(ele.P4()) <= dr)
      return true;
  return false;
}
...
void ZSelector(const std::vector<T>& LeptonList, std::vector<ZSpace::ZCandidate>& candList) {
  for (unsigned int i = 0; i < LeptonList.size(); ++i) {
	...
	ztmp.flavour = static_cast<int>(ZSpace::ZType::unkwn);
      ...
      if (ztmp.massDiff < 10) candList.push_back(ztmp);
    }
  }
}
