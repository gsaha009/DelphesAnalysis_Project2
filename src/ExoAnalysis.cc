...
void ExoAnalysis::bookHistograms()
{
  ...
  // Raw Muon 
  new TH1D("Raw_mu_pt",       "Raw #mu p_{T} (GeV)",     100, 0, 300);
  new TH1D("Raw_mu_eta",      "Raw #mu #eta",            50, -5, 5);
  new TH1D("Raw_mu_phi",      "Raw #mu #phi",            32, -3.2, 3.2);
  new TH1D("Raw_mu_charge",   "Raw #mu charge",          3, -1.5, 1.5);
  new TH1D("Raw_mu_isolationVar",        "Raw #mu Iso",  100, 0, 5);
  new TH1D("Raw_mu_isolationVarRhoCorr", "Raw #mu Iso #rho Corr", 100, 0, 5);
  new TH1D("Raw_mu_D0",       "Raw #mu D0",              40, -2, 2);
  new TH1D("Raw_mu_DZ",       "Raw #mu DZ",              40, -2, 2);
  // Raw Electron
  new TH1D("Raw_ele_pt",       "Raw #ele p_{T} (GeV)",     100, 0, 300);
  new TH1D("Raw_ele_eta",      "Raw #ele #eta",            50, -5, 5);
  new TH1D("Raw_ele_phi",      "Raw #ele #phi",            32, -3.2, 3.2);
  new TH1D("Raw_ele_charge",   "Raw #ele charge",          3, -1.5, 1.5);
  new TH1D("Raw_ele_isolationVar",        "Raw #ele Iso",  100, 0, 5);
  new TH1D("Raw_ele_isolationVarRhoCorr", "Raw #ele Iso #rho Corr", 100, 0, 5);
  new TH1D("Raw_ele_D0",       "Raw #ele D0",              40, -2, 2);
  new TH1D("Raw_ele_DZ",       "Raw #ele DZ",              40, -2, 2);
  // Raw Jet
  new TH1D("Raw_jet_pt",       "Raw #jet p_{T} (GeV)",     100, 0, 300);
  new TH1D("Raw_jet_eta",      "Raw #jet #eta",            50, -5, 5);
  new TH1D("Raw_jet_phi",      "Raw #jet #phi",            32, -3.2, 3.2);
  new TH1D("Raw_jet_mass",     "Raw #jet mass",            100, 0, 50);
  new TH1D("Raw_jet_DeltaEta", "Raw #jet radius in pseudorapidity",     50, -5, 5);
  new TH1D("Raw_jet_DeltaPhi", "Raw #jet radius in azimuthal angle",    32, -3.2, 3.2);
  new TH1D("Raw_jet_flavor",   "Raw #jet flavor",          10, -0.5, 9.5);
  new TH1D("Raw_jet_tautag",   "Raw #jet tautag",          3, -1.5, 1.5);
  new TH1D("Raw_jet_tauwt",    "Raw #jet probability for jet to be identified as #tau",  50, 0, 1);
  new TH1D("Raw_jet_EhadOverEem", "Raw #jet ratio of the hadronic versus electromagnetic energy deposited in the calorimeter", 100, 0, 5);
  new TH1D("Raw_jet_BTag",     "Raw #jet btag",              2, -0.5, 1.5);
  new TH1D("Raw_jet_BetaStar", "Raw #jet BetaStar (sum pt of charged constituents coming from hard interaction)/(sum pt of charged constituents)", 100, 0, 5);
  ...
  if (isDL()) {
    ...
    new TH1D("evtCutFlow", "Event CutFlow", 9, -0.5, 8.5);
    new TH1D("evtCutFlowWt", "HepMCEvent Weight", 9, -0.5, 8.5);
  }
  else
    std::cerr << "No channel mentioned. Please mention either SL or DL. no EvtCutFlow hist is booked ... "
	      << std :: endl;
  ...
  // tauh
  new TH1D("Tauh_Pt",  "pT of 1st Tauh",  40, 0.0, 500.0);
  new TH1D("Tauh_Eta", "Eta of 1st Tauh", 40, 0.0, 5.0);
  new TH1D("Tauh_Phi", "Phi of 1st Tauh", 40, 0.0, 3.3);
  // bjet
  ...
  // tauh-leadbj 
  new TH1D("DR_tauh_leadbj",   "#DeltaR (#tau_{h}, lead b-Jet)",    25, 0, 5);
  new TH1D("DPhi_tauh_leadbj", "#Delta#phi (#tau_{h}, lead b-Jet)", 32, -3.2, 3.2);
  new TH1D("DEta_tauh_leadbj", "#Delta#eta (#tau_{h}, lead b-Jet)", 50, -5, 5);
  // tauh-leadlj
  new TH1D("DR_tauh_leadlj",   "#DeltaR (#tau_{h}, lead light-Jet)",    25, 0, 5);
  new TH1D("DPhi_tauh_leadlj", "#Delta#phi (#tau_{h}, lead light-Jet)", 32, -3.2, 3.2);
  new TH1D("DEta_tauh_leadlj", "#Delta#eta (#tau_{h}, lead light-Jet)", 50, -5, 5);
  // tauh-leadj
  new TH1D("DR_tauh_leadj",    "#DeltaR (#tau_{h}, lead Jet)",    25, 0, 5);
  new TH1D("DPhi_tauh_leadj",  "#Delta#phi (#tau_{h}, lead Jet)", 32, -3.2, 3.2);
  new TH1D("DEta_tauh_leadj",  "#Delta#eta (#tau_{h}, lead Jet)", 50, -5, 5);
  // met-tauh,leadbj,leadlj 
  new TH1D("DPhi_met_tauh",    "#Delta#phi (#slash{E}_{T}, #tau_{h})", 32, -3.2, 3.2);
  new TH1D("DPhi_met_leadbj",  "#Delta#phi (#slash{E}_{T}, lead b-Jet)", 32, -3.2, 3.2);
  new TH1D("DPhi_met_leadlj",  "#Delta#phi (#slash{E}_{T}, lead light-Jet)", 32, -3.2, 3.2);
  // leadlj-leadbj 
  new TH1D("DR_leadbj_leadlj",   "#DeltaR (lead b-Jet, lead light-Jet)", 25, 0, 5);
  new TH1D("DPhi_leadbj_leadlj", "#Delta#phi (lead b-Jet, lead light-Jet)", 32, -3.2, 3.2);
  new TH1D("DEta_leadbj_leadlj", "#Delta#eta (lead b-Jet, lead light-Jet)", 50, -5, 5);

  new TH1D("PT_RecoTau",  "Reconstructed #tau p_{T} (GeV) : Collinear approx", 50, 0, 500);
  new TH1D("Eta_RecoTau", "Reconstructed #tau #eta : Collinear approx",        25, -2.5, 2.5);
  ...
  if (isDL()) {
    ...
    new TH1D("MET_along_tauh", "#slash{E}_{T} along #tau_{h}",                 40, 0., 200.);
    new TH1D("FracMetAlongTauh", "#slash{E}_{T} along #tau_{h}/#slash{E}_{T}", 50, 0, 1);
    new TH1D("DR_xlep_tauh",     "#DeltaR (#mu from #chi, #tau_{h})",          25, 0, 5);
    new TH1D("DPhi_xlep_tauh",   "#Delta#phi (#mu from #chi, #tau_{h})",       32, -3.2, 3.2);
    new TH1D("DEta_xlep_tauh",   "#Delta#eta (#mu from #chi, #tau_{h})",       50, -5, 5);
    new TH1D("DR_xlep_leadlj",   "#DeltaR (#mu from #chi, lead light-Jet)",         25, 0, 5);
    new TH1D("DPhi_xlep_leadlj", "#Delta#phi (#mu from #chi, lead light-Jet)",      32, -3.2, 3.2);
    new TH1D("DEta_xlep_leadlj", "#Delta#eta (#mu from #chi, lead light-Jet)",      50, -5, 5);
    new TH1D("DR_xlep_leadj",    "#DeltaR (#mu from #chi, lead Jet)",         25, 0, 5);
    new TH1D("DPhi_xlep_leadj",  "#Delta#phi (#mu from #chi, lead Jet)",      32, -3.2, 3.2);
    new TH1D("DEta_xlep_leadj",  "#Delta#eta (#mu from #chi, lead Jet)",      50, -5, 5);
    new TH1D("DPhi_xlep_met",    "#Delta#phi (#mu from #chi, #slash{E}_{T})", 32, -3.2, 3.2);
    new TH1D("EffectiveMass_t1", "Effective mass of top (GeV)",               50, 0, 300);
    new TH1D("InvMass_X_test",   "Invariant mass of #chi (GeV) : improper",   50, 0, 200);
    new TH1D("MT_X",             "Transverse mass of #chi (GeV)",             50, 0, 200);
    new TH1D("InvM_coln_XlepTauh", "Collinear mass (GeV)",                    50, 0, 200);
    new TH1D("InvM_coln_XlepTauh_MuMu", "Collinear mass (GeV) : #mu#mu",      50, 0, 200);
    new TH1D("InvM_coln_XlepTauh_EleMu", "Collinear mass (GeV) : ele#mu",     50, 0, 200);
    new TH1D("MT_xlep_met", "Transverse mass (#mu from #chi, #slash{E}_{T}) (GeV)", 50, 0, 200);

    new TH1D("WlepPt",  "W#pm lepton p_{T} (GeV)", 50, 0., 200.);
    new TH1D("WlepEta", "W#pm lepton #eta",        25, -2.5, 2.5);
    new TH1D("DR_wlep_leadbj",    "#DeltaR (lepton from W, lead b-Jet)",    25, 0, 5);
    new TH1D("DPhi_wlep_leadbj",  "#Delta#phi (lepton from W, lead b-Jet)", 32, -3.2, 3.2);
    new TH1D("DEta_wlep_leadbj",  "#Delta#eta (lepton from W, lead b-Jet)", 50, -5, 5);
    new TH1D("MT_wlep_met",       "M_{T} (lepton from W, #slash{E}_{T}) (GeV)",50, 0, 200);
    new TH1D("InvM_coln_WlepTauh","Collinear mass of lepton from W and #tau (GeV) : Test", 50, 0, 200);      

    new TH1D("Scalarsumpt_xlep_wlep", "Scalar sum p_{T} (GeV)", 50, 0, 400);
    new TH1D("Vectorsumpt_xlep_wlep", "Vector sum p_{T} (GeV)", 50, 0, 400);
    new TH1D("DR_xlep_wlep",          "#DeltaR", 25, 0, 5);
    new TH1D("DPhi_xlep_wlep",        "#Delta#phi", 32, -3.2, 3.2);
    new TH1D("DEta_xlep_wlep",        "#Delta#eta", 50, -5, 5);
    new TH1D("DR_xlep_leadbj",        "#DeltaR", 25, 0, 5);
    new TH1D("DPhi_xlep_leadbj",      "#Delta#phi", 32, -3.2, 3.2);
    new TH1D("DEta_xlep_leadbj",      "#Delta#eta", 50, -5, 5);
    new TH1D("DR_wlep_leadlj",        "#DeltaR", 25, 0, 5);
    new TH1D("DPhi_wlep_leadlj",      "#Delta#phi", 32, -3.2, 3.2);
    new TH1D("DEta_wlep_leadlj",      "#Delta#eta", 50, -5, 5);
    new TH1D("DR_wlep_tauh",          "#DeltaR", 25, 0, 5);
    new TH1D("DPhi_wlep_tauh",        "#Delta#phi", 32, -3.2, 3.2);
    new TH1D("DEta_wlep_tauh",        "#Delta#eta", 50, -5, 5);
    new TH1D("Scalarsumpt_vis",       "Scalar sum p_{T} (GeV) visible", 50, 0, 500);
    new TH1D("Scalarsumpt",           "Scalar sum p_{T} (GeV)", 50, 0, 500);
    new TH1D("Effectivemass_vis",     "Effectivemass_vis (GeV)", 50, 0, 500);
    new TH1D("Effectivemass",         "Effectivemass (GeV)", 50, 0, 500);

    new TH1D("HT_Jets",           "H_{T} (GeV)",   50, 0, 400);
    new TH1D("minDr_XlepJets",    "minimum #Delta R [#mu from #chi, all jets]",    25, 0, 5.0);
    new TH1D("maxDr_XlepJets",    "maximum #Delta R [#mu from #chi, all jets]",    25, 0, 5.0);
    new TH1D("minDphi_XlepJets",  "minimum #Delta #Phi [#mu from #chi, all jets]", 25, 0, 5.0);
    new TH1D("maxDphi_XlepJets",  "maximum #Delta #Phi [#mu from #chi, all jets]", 25, 0, 5.0);
    new TH1D("minDr_WlepJets",    "minimum #Delta R [lepton from W, all jets]",    25, 0, 5.0);
    new TH1D("maxDr_WlepJets",    "maximum #Delta R [lepton from W, all jets]",    25, 0, 5.0);
    new TH1D("minDphi_WlepJets",  "minimum #Delta #Phi [lepton from W, all jets]", 25, 0, 5.0);
    new TH1D("maxDphi_WlepJets",  "maximum #Delta #Phi [lepton from W, all jets]", 25, 0, 5.0);
  ...
  // ------------------------------------------------------------------------------ //
    ...
    new TH1D("w_massdiff",    "mass diff to check w jets",             25, -100, 100);
    // book histograms for one leg
    new TH1D("Wj1Pt",         "p_{T} (GeV) of leading-jet from W",     20, 0, 300);
    new TH1D("Wj2Pt",         "p_{T} (GeV) of subleading-jet from W",  20, 0, 300);
    new TH1D("WPt",           "p_{T} (GeV) of W",                      20, 0, 300);
    new TH1D("InvM_Wj1Wj2",   "Invariant mass [W-jet1, W-jet2] (GeV)", 50, 0, 200);
    new TH1D("DR_Wj1Wj2",     "#Delta R [W_{j1}, W_{j2}]",             25, 0, 5);
    new TH1D("DPhi_Wj1Wj2",   "#Delta #Phi [W_{j1}, W_{j2}]",          32, -3.2, 3.2);
    new TH1D("DEta_Wj1Wj2",   "#Delta #eta [W_{j1}, W_{j2}]",          25, -5, 5);
    new TH1D("DR_w_leadb",    "#Delta R [W, leading b-jet]",           25, 0, 5);
    new TH1D("DPhi_w_leadb",  "#Delta #Phi [W, leading b-jet]",        32, -3.2, 3.2);
    new TH1D("DR_wj1_leadb",  "#Delta R [W_{j1}, leading b-jet]",     25, 0, 5);
    new TH1D("DPhi_wj1_leadb","#Delta #Phi [W_{j1}, leading b-jet]",  32, -3.2, 3.2);
    new TH1D("DR_wj2_leadb",  "#Delta R [W_{j2}, leading b-jet]",     25, 0, 5);
    new TH1D("DPhi_wj2_leadb","#Delta #Phi [W_{j2}, leading b-jet]",  32, -3.2, 3.2);
    new TH1D("InvM_w_leadbj", "Invariant Mass [W, leading b-jet] (GeV)", 50, 0, 500);
    // book histograms for another leg
    new TH1D("XlepPt",          "p_{T} of #mu from #chi",         50, 0, 200);
    new TH1D("XlepEta",         "#eta of #mu from #chi",          32, -3.2, 3.2);
    new TH1D("M_coll_Xlep",     "M_{coll} (GeV)",                 200, 0, 1000);
    new TH1D("M_coll_test",     "M_{coll} (GeV)",                 200, 0, 1000);
    new TH1D("InvM_Xleadlj",    "Invariant mass [#chi, leading light-jet] (GeV)",  100, 0, 1000);
    new TH1D("DR_Xlep_leadlj",  "#Delta R [#mu from #chi, leading light-jet]",     25, 0, 5);
    new TH1D("DPhi_Xlep_leadlj","#Delta #Phi [#mu from #chi, leading light-jet]",  32, -3.2, 3.2);
    new TH1D("DR_Xtauh_leadlj", "#Delta R [#tau_{h} from #chi, leading light-jet]", 25, 0, 5);
    new TH1D("DPhi_Xtauh_leadlj","#Delta #Phi [#tau_{h} from #chi, leading light-jet]", 32, -3.2, 3.2);
    new TH1D("DR_X_leadlj",     "#Delta R [#chi, leading light-jet]",        25, 0, 5);
    new TH1D("DPhi_X_leadlj",   "#Delta #Phi [#chi, leading light-jet]",     32, -3.2, 3.2);
    new TH1D("DR_Xlep_Xtau",    "#Delta R [(#mu, #tau) form #chi]",          25, 0, 5);
    new TH1D("DPhi_Xlep_Xtau",  "#Delta #Phi [(#mu, #tau) form #chi]",       32, -3.2, 3.2);
    // communication between 2 legs
    new TH1D("DR_Xlep_wj1",     "#Delta R",             25, 0, 5);     
    new TH1D("DPhi_Xlep_wj1",   "#Delta #Phi",          32, -3.2, 3.2);
    new TH1D("DEta_Xlep_wj1",   "#Delta #eta",          32, -3.2, 3.2);    
    new TH1D("DR_Xlep_wj2",     "#Delta R",             25, 0, 5);
    new TH1D("DPhi_Xlep_wj2",   "#Delta #Phi",          32, -3.2, 3.2);
    new TH1D("DEta_Xlep_wj2",   "#Delta #eta",          32, -3.2, 3.2);    
    new TH1D("DR_Xlep_w",       "#Delta R",             25, 0, 5); 
    new TH1D("DPhi_Xlep_w",     "#Delta #Phi",          32, -3.2, 3.2);    
    new TH1D("DEta_Xlep_w",     "#Delta #eta",          32, -3.2, 3.2);    
    new TH1D("DR_Xlep_leadbj",       "#Delta R",             25, 0, 5);
    new TH1D("DPhi_Xlep_leadbj",     "#Delta #Phi",          32, -3.2, 3.2);    
    new TH1D("DEta_Xlep_leadbj",     "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_Xtau_wj1",     "#Delta R",             25, 0, 5);
    new TH1D("DPhi_Xtau_wj1",   "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_Xtau_wj1",   "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_Xtau_wj2",     "#Delta R",             25, 0, 5);
    new TH1D("DPhi_Xtau_wj2",   "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_Xtau_wj2",   "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_Xtau_w",       "#Delta R",             25, 0, 5);
    new TH1D("DPhi_Xtau_w",     "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_Xtau_w",     "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_Xtau_leadbj",       "#Delta R",             25, 0, 5);
    new TH1D("DPhi_Xtau_leadbj",     "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_Xtau_leadbj",     "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_leadbj_leadlj",       "#Delta R",             25, 0, 5);
    new TH1D("DPhi_leadbj_leadlj",     "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_leadbj_leadlj",     "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_w_leadlj",       "#Delta R",             25, 0, 5);
    new TH1D("DPhi_w_leadlj",     "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_w_leadlj",     "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_X_leadbj",          "#Delta R",             25, 0, 5);
    new TH1D("DPhi_X_leadbj",        "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_X_leadbj",        "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_X_w",          "#Delta R",             25, 0, 5);
    new TH1D("DPhi_X_w",        "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_X_w",        "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("DR_t1_t2",        "#Delta R",             25, 0, 5);
    new TH1D("DPhi_t1_t2",      "#Delta #Phi",          32, -3.2, 3.2); 
    new TH1D("DEta_t1_t2",      "#Delta #eta",          32, -3.2, 3.2); 
    new TH1D("Total_mass",      "mass (GeV)",           100, 0, 2000); 
    new TH1D("Total_vector_pt", "vec sum p_{T} (GeV)",  40, 0, 500); 
    new TH1D("Total_scalar_pt", "scalar sum p_{T} (GeV)", 100, 0, 2000); 
    // some more high level variables
    new TH1D ("HT_Jets",         "H_{T} (GeV)",                100, 0, 2000);
    new TH1D ("minDr_XlepJets",  "min #Delta R [Xlep, jets]",  25, 0, 5);
    new TH1D ("maxDr_XlepJets",  "max #Delta R [Xlep, jets]",  25, 0, 5);
    new TH1D ("minDphi_XlepJets","minDphi_XlepJets",           32, 0, 3.2);
    new TH1D ("maxDphi_XlepJets", "maxDphi_XlepJets",          32, 0, 3.2);
  }
  else std::cerr 
	 << "No Channel is provided. Please give SL or DL. Histograms for channel arent booked"
	 << std::endl;
...
void ExoAnalysis::eventLoop(ExRootTreeReader *treeReader)
{
  ...
  for (size_t iEntry = 0; iEntry < nEvents; ++iEntry) {
    ...
    if (isDL()) {
      ...
      if (MuMu) { 
	// need a tagger? 
	XLepP4 = (isLowMassX()) ? LepColl[1].P4 : LepColl[0].P4;
	WLepP4 = (isLowMassX()) ? LepColl[0].P4 : LepColl[1].P4;
      }
      else if (EleMu) {
	if (LepColl[0].Flavour == 1) {
      ...
      if (MuMu)       AnaUtil::fillHist1D("InvM_coln_XlepTauh_MuMu", M_coll_Xlep);
      else if (EleMu) AnaUtil::fillHist1D("InvM_coln_XlepTauh_EleMu", M_coll_Xlep);
      else            std::cerr << "Wrong category ... not MuMu or EleMu \n";
